...
execute 'download docker-compose' do
  command "curl -sL https://github.com/docker/compose/releases/download/#{node['sysdig_workstation']['docker_compose']['version']}/docker-compose-#{node['kernel']['name']}-#{node['kernel']['machine']} -o /usr/local/bin/docker-compose"
